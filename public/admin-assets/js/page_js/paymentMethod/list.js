$(document).ready(function () {
	var paymentMethodTable = $("#payment_method_table").DataTable({
		...
		ajax: {
			...
			beforeSend: function () {
				if (paymentMethodTable != null) {
					paymentMethodTable.settings()[0].jqXHR.abort();
				}
			},
		...
		columnDefs: [
			...
			},
			{
				...
				render: function (data, type, full, meta) {
					var deleteHtml = "";
					if (!jQuery.inArray(meta.row, ["0", "1", "2"])) {
						deleteHtml =
					...
					return (
						'<a href="' +
						full.edit_url +
						'" class="btn btn-sm btn-icon btn-light-primary btn-active-primary" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-inverse" data-bs-placement="top" title ="Edit Payment Method">\
							<span class="svg-icon">\
								<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">\
	...
	$("#datatable_payment_method_search").on("keyup", function (e) {
		paymentMethodTable.search(e.target.value).draw();
	});
	$("input[type=search]").on("search", function () {
		paymentMethodTable.search("").draw();
	});
	const documentTitle = "Payment Method Report";
	var buttons = new $.fn.dataTable.Buttons(paymentMethodTable, {
		buttons: [
			{
				extend: "csvHtml5",
				title: documentTitle,
		...
		if (result.value) {
			...
			destroyFunctionAjax = $.ajax({
				...
				success: function (resultData) {
					$("#payment_method_table").DataTable().ajax.reload();
					Toast.fire({
						icon: "success",
						title:
